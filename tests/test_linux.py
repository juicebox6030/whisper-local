...
def test_extension_install_uses_packaged_resources():
    import json
    import zipfile
    from whisper_key.platform.linux import permissions, bridge
    def run(command, **kwargs):
        if command[1] == 'install':
            with zipfile.ZipFile(command[-1]) as archive:
                assert set(archive.namelist()) == {'extension.js', 'metadata.json'}
                assert json.loads(archive.read('metadata.json'))['uuid'] == bridge.UUID
                assert b'export default class WhisperLocal' in archive.read('extension.js')
        return Mock(returncode=0)
    with patch.object(permissions.subprocess, 'run', side_effect=run) as command:
        assert permissions.install_extension() == 0
        assert command.call_count == 2
        assert command.call_args.args[0] == ['gnome-extensions', 'enable', bridge.UUID]


def test_extension_install_failure_and_deferred_activation(capsys):
    from whisper_key.platform.linux import permissions
    with patch.object(permissions.subprocess, 'run', side_effect=FileNotFoundError('gnome-extensions')):
        assert permissions.install_extension() == 1
    with patch.object(permissions.subprocess, 'run', return_value=Mock(returncode=1)):
        assert permissions.install_extension() == 0
    assert 'After logging in' in capsys.readouterr().out


@pytest.mark.parametrize('remove_extension', [False, True])
def test_extension_uninstall_requires_separate_consent(tmp_path, monkeypatch, remove_extension):
    from whisper_key import uninstall, autostart
    from whisper_key.platform.linux.bridge import UUID
    monkeypatch.setenv('XDG_DATA_HOME', str(tmp_path))
    extension = tmp_path / 'gnome-shell' / 'extensions' / UUID
    extension.mkdir(parents=True)
    with patch.object(uninstall, 'get_user_app_data_path', return_value=str(tmp_path / 'config')), \
         patch.object(uninstall, '_our_cached_models', return_value=[]), \
         patch.object(autostart, 'is_enabled', return_value=False), \
         patch.object(uninstall, '_confirm', side_effect=[True, remove_extension]), \
         patch('subprocess.run') as command:
        assert uninstall.run_uninstall() == 0
        if remove_extension:
            command.assert_called_once_with(['gnome-extensions', 'uninstall', UUID], check=True)
        else:
